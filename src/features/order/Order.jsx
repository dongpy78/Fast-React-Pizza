  ...
  return (
    <div className="px-4 py-6 space-y-8 ">
      ...
      <ul className="dive-stone-200 divide-y border-b border-t ">
        {cart.map((item) => (
          <OrderItem item={item} key={item.pizaaId} isLoadingIngredients={fetcher.state === "loading" && !fetcher.data} ingredients={fetcher?.data?.find((el) => el.id === item.pizzaId)?.ingredients ?? []}/>
        ))}
      </ul>
